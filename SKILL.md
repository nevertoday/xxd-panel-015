---
name: xxd-panel-015
description: "Create XXD Panel 015 artwork from supplied photos in four combinable modes: photo above/minimal abstract-memory editorial below, photo left/design right, design alone, or a four-device wallpaper pack with independent or anchor-linked continuity. Deconstructs, filters, distils, and reconstructs source identity through a few geometric forms that preserve distinctive mass ratios, axes, curves, interlocks, layers, occlusion, negative space, and asymmetry, using a strict source-derived palette and art-book microtypography. Use for the exact 015 modern quiet collectible abstraction; never use it for generic icons, meaningless geometry, evenly distributed shapes, fragmented collage, commercial templates, or multi-photo collage."
---

# XXD Panel 015 · 极简抽象记忆编辑
...
Modes and device ratios may change placement and aspect ratio but never the 015 motive. Every transformed frame must visibly express this chain:
...
## Copy and typography belong to 015

015 text is a complete art-book microtypographic system that gives the abstraction structure, rhythm, and reading order—not a detached headline, generic caption, UI overlay, or pseudo-intellectual filler.
...
Typography has its own 015 construction:
...
Use only images attached to the current invocation, explicit user paths, or a previously supplied image that the current request clearly identifies as “the same image”. Never broadly scan the Desktop, workspace, `~/Desktop/xxd/xxd-panel-015/`, or historical task folders to find a substitute source. If the intended source cannot be accessed, ask for it.
...
1. Resolve one or more modes. In a native multi-select use these human-facing labels: `上下对照`, `左右对照`, `只要设计图`, and `四端壁纸`. If no real multi-select exists, ask and wait:
   ...
   1. 上下对照（原图在上，015 设计在下）
   2. 左右对照（原图在左，015 设计在右）
   3. 只要设计图（成品中不显示原图）
   4. 四端壁纸
      手机、iPad、电脑、儿童手表各一张
...
5. Start a fresh job and reserve collision-safe output directories before generation. Use only the current invocation's explicit source or theme. Read `references/xxd-panel-015-prompt.en.md` or `references/xxd-panel-015-prompt.zh-CN.md` immediately before building the generation request.
...
7. Use **single complete-canvas generation as the default for every mode**:
   - `top-bottom`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source in the upper half and the 015 transformation in the lower half.
   - `left-right`: supply the source as a high-fidelity edit/reference input and generate one finished image containing the faithful source on the left and the 015 transformation on the right.
   - `design-only`: generate the 015 transformation across the whole canvas; the source is reference-only and not visible.
...
10. Inspect the actual bitmap at full size and thumbnail size. Check, in order: whole-poster integration; 015 aesthetic fidelity; source identity and structure; visual and semantic correspondence between regions; typography and locale; mode, ratio/pixels, count, and PNG format; then approximate 50/50 geometry.
...
COPY PAYLOAD: <the exact 015-specific title and supporting-text package resolved under the local production prompt>
COPY RULE: Render only the populated strings in COPY PAYLOAD, each exactly once. Do not rewrite, translate, spell-correct, duplicate, or add text. Use native shaping, direction, punctuation, spacing, and semantic line breaks. Preserve the 015-specific hierarchy, amount of supporting text, placement, material, and typographic role instead of applying a generic overlay.
```
...
Save finished work under `~/Desktop/xxd/xxd-panel-015/` unless the user supplies another destination. Create the shared `~/Desktop/xxd/` wrapper, the skill root, and each task directory when needed.
...
- Automatic copy is concise, grounded, and 015-specific; exact user wording remains verbatim. All rendered text is accurate and native to the resolved locale; text-free output contains no text or pseudo-text.
...
User instructions may change subject emphasis, selected structural cues, form vocabulary, mass ratio, positive/negative balance, colour roles, type path, title voice, or microtext mix within 015, but do not silently relax one-photo isolation, exact paired geometry, four separate wallpaper outputs, fresh-task generation, source-hidden output rules, native-language typography, few-form discipline, or source-derived palette roles. Leave the minimal abstract-memory editorial aesthetic only when the user explicitly asks to leave the 015 style.

## Provenance boundary

The user's original style brief is archived at [references/015-source.md](references/015-source.md). It records the initial 3:4 top-bottom example but does not override the operative mode and explicit final-canvas selection and complete-canvas generation rules. The full local 015 prompt is the production specification. Never borrow subjects, colours, copy, or compositions from samples or previous outputs.
